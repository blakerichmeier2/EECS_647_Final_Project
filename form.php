  ...
  <body class = "p-3 mb-2 bg-dark text-white align-center">
    <form class="form-horizontal" role="form" method="post" action="form.php">
       <div class="row offset-1">
          <div class="col">
              <input type="text" name="class_code" id="class_code" class="form-control" placeholder="(search class name)" aria-label="Class Name">
          </div>
          <div class = "col">
          <select class="form-select form-select-md mb-3" name="class_term" id = "class_term" aria-label=".form-select-lg example">
              <option vlaue="Any">Any Semester</option>
...
<?php
    ...
    if($class_code == "" && $class_term == "Any Semester" && $class_instruction_type == "Any Instruction Mode"){
    ...
    else if($class_code != "" && $class_term == "Any Semester" && $class_instruction_type == "Any Instruction Mode"){
      $query = "SELECT ClassCode, PROFESSOR.name, BuildingName, RoomNumber, CreditHours, SeatsAvailable, Time, Mode, ClassTerm FROM CLASS, TEACHES, PROFESSOR 
    ...
    echo '<br>';
    echo "<table border='1'>";
    echo "<tr><th>Class Code</th><th>Professor</th><th>Instruction Mode</th><th>Building Name</th><th>Building Number</th><th>Credit Hours</th><th>Seats Available</th><th>Time</th><th>Term</th></tr>";
